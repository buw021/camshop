...
const ProductForm: React.FC<{
  ...
  onProductAdded: () => void;
}> = ({ toggleAddProduct, setIsDirty, onProductAdded }) => {
  const {
    product,
    setProduct,
    addValue,
  ...
  const deleteUploadedFiles = async (filePaths: string[]) => {
    try {
      await axiosInstance.post("/delete-uploaded-files", { filePaths });
  ...
  useEffect(() => {
    if (product) {
      setIsDirty(!isProductEmpty(product));
    }
  }, [product, isProductEmpty, setIsDirty]);
  ...
  const checkBeforeSubmit = (): boolean => {
    ...
    product.variants.forEach((variant, index) => {
      ...
      if (
        variant.previewUrl.length === 0 ||
        (variant.selectedImgFiles && variant.selectedImgFiles.length === 0)
      ) {
  ...
  const addProduct = async (fileImgs: string[]) => {
    try {
  ...
  const confirm = async () => {
    ...
    if (check) {
      ...
      if (fileUploadSuccess) {
        await addProduct(fileUploadSuccess);
      }
    }
  };
  ...
  return (
    <form className="relative flex h-full w-full flex-col gap-2 overflow-hidden rounded-lg bg-white px-6 py-4 text-xs">
      ...
      <div
        className={` ${activeMenu !== "product" && "hidden"} self-end text-sm`}
      >
        <button
          type="button"
